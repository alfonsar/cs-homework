...
char* flexCharManager::alloc_chars(int n){
	...
	{
		Mem_Block beginning (n,buffer);
		active_requests++;
		free_mem=free_mem-n;
	   return beginning.physical_location;
	}
	
	else
	{
		int enough=*used_memory[0]->size;
		if(n<=enough)
		{
			Mem_Block another (n,buffer);
			active_requests++;
			free_mem=free_mem-n;
			return another.physical_location;
		}
	}
	else 
	{
		...
		{
			int space=*used_memory[i+1]->physical_location-*used_memory[i]->physical_location-*used_memory[i]->size;
			if(n<=space)
			{
				Mem_Block another2 (n,buffer);
				active_requests++;
				free_mem=free_mem-n;
				return another2.physical_location;
			}
		}
	}
*/
...
void flexCharManager::free_chars(char* p)
{
	...
	for (int i=0; i<active_requests;i++)
	{
		if(p==used_memory[i]->physical_location)
		{
			...
			place=(used_memory[i]->physical_location-&buffer[i])/sizeof(char);
